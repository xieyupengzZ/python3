...
def datetimeTest():
    ...
    print('字符串 → datetime，时间字符串，日期只能间隔横杆，时间只能间隔冒号',dt.fromisoformat('2020-07-04 17:39:30'))
    print('字符串 → datetime，时间字符串和格式字符串，同上',dt.strptime('2020-07-04 17:39:30','%Y-%m-%d %H:%M:%S'))
    print('datetime → 字符串，可间隔任意字符',today.strftime('%Y/%m/%d %H/%M/%S'),today.strftime('%Y-%m-%d %H:%M:%S'))
    print('datetime → 字符串，默认间隔是T，可替换；默认是微秒，可替换成毫秒',today.isoformat(),today.isoformat(' ','milliseconds'))
